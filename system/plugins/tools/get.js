const undici = require("node-fetch");
...
module.exports = {
  ...
  async run(m, { sock, Func, text, config }) {
    if (!text) return m.reply("> ❌ Masukan atau reply URL yang ingin kamu ambil datanya");

    const urls = isUrl(text);
    if (!urls) return m.reply("> ❌ Format URL tidak valid, pastikan URL yang dimasukkan benar");
