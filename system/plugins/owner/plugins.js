...
module.exports = {
    ...
    async run(m, {
        ...
        if (text.includes("--get")) {
            ...
            if (!input)
                return m.reply(`> Mohon pilih plugin dengan menyertakan nomor atau nama plugin.`);
            ...
            if (!m.quoted || !m.quoted.body)
                return m.reply(`> *– 乂 Mohon balas pesan berisi kode plugin yang ingin kamu simpan.*\n> Harap pastikan bahwa kode plugin yang kamu kirim valid dan lengkap!`);

            let input = text.replace("--add", "").trim();
            if (!input) return m.reply(`> Masukkan nama plugin yang ingin kamu tambahkan.`);
            ...
            if (!input)
                return m.reply(`> Silakan masukkan nama atau nomor plugin yang ingin kamu hapus.`);
