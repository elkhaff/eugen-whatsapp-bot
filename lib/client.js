(async () => {
  ...
  process.on("message", (msg) => {
    if (msg?.type !== "reload" || !msg.filename) return;
    try {
      const resolved = require.resolve(msg.filename);
      delete require.cache[resolved];
      require(msg.filename);
      console.log(
        chalk.greenBright(`[ HOT RELOAD ] `) +
          chalk.white(
            `${require("node:path").relative(process.cwd(), msg.filename)} berhasil di-reload.`,
          ),
      );
    } catch (e) {
      console.error(chalk.redBright(`[ HOT RELOAD ERROR ] `) + e.message);
    }
  });
  ...
  */
  ...
  console.log(
    chalk.white.bold("  | Versi: ") + chalk.magenta.bold(pkg.version),
  );
  console.log(
    chalk.white.bold("  | Versi Node.js: ") +
      chalk.magenta.bold(process.version),
  );
  ...
  async function system() {
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      if (connection === "close") {
        ...
        console.log(
          chalk.red.bold(
            `Koneksi ditutup karena (Code: ${lastDisconnect.error.output.statusCode}): `,
          ),
        ...
        if (reason === DisconnectReason.badSession) {
          ...
          console.log(
            chalk.yellow.bold(
              "Koneksi ditutup, mencoba menghubungkan kembali...",
            ),
          );
          system();
          ...
          console.log(
            chalk.yellow.bold(
              "Koneksi internet hilang, mencoba menghubungkan kembali...",
            ),
          );
          system();
        } else if (reason === DisconnectReason.connectionReplaced) {
          console.log(
            chalk.green.bold("Koneksi diganti, sesi lain telah dibuka."),
          );
          process.exit(0);
          ...
          console.log(
            chalk.magenta.bold(
              "Koneksi diputus paksa (428), mencoba masuk kembali...",
            ),
          );
          system();
        } else {
          console.log(
            chalk.white.bold(
              `Terputus dengan alasan tidak diketahui (${reason}), mencoba menyambung...`,
            ),
          );
          system();
        }
        ...
        try {
          ...
          log.info(
            `📁 Group metadata loaded and cached: ${Object.keys(sock.groupData).length}`,
          );
        } catch (e) {
          log.error("❌ Gagal ambil metadata grup:", e.message);
        }

        require("./cronjobs.js")(db, sock, config);
      }
    });
...
Terima kasih telah menambahkan Eugen ke dalam grup. Jika ada pertanyaan, silakan tanyakan kepada Owner.`;
          ...
          log.error(
            `Terjadi kesalahan pada event groups.upsert untuk grup ${group.id}:`,
            err,
          );
        }
      }
    ...
    sock.ev.on(
      ...
      async ({ id, participants, action }) => {
        ...
        const resolveJid = (input) => {
          let found = chat.participants.find(
            (p) => p.id === input || p.lid === input,
          );
        ...
        if (participants[0] !== sock.user.lid.split(":")[0] + "@lid") {
          ...
          log.info(
            `Metadata fetched for group ${id} (${metadata.subject}) on participant update.`,
          );
        }
          ...
          case "add":
            if (group.welcome?.status) {
              for (let user of participants) {
                ...
                console.log(realJid);
                let exists = chat.participants.findIndex(
                  (p) => p.id === realJid,
                );
                if (exists === -1) {
