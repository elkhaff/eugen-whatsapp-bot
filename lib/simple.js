          ...
          ...(mime === "image"
            ...
            : mime === "video"
              ...
              : {
                  ...
                  mimetype: file.mime,
                  fileName:
                    json.filename || "Eugen." + extension(file.mime),
                }),
        },
        {
...
sock.sendStatus = async(content, jids) => {
  ...
  sock.getName = async (jidOrLid = "", withoutContact = false) => {
    const getJidFromLid = (input) => {
        for (let gId in sock.groupData) {
            let chat = sock.groupData[gId];
    ...
    let jid = getJidFromLid(jidOrLid);
    ...
    const getSafePhoneNumber = (id) => {
        try {
            const num = id.split('@')[0];
            if (!isNaN(num) && num.length > 5 && num.length < 16) {
                return PhoneNumber("+" + num).getNumber("international");
            }
            return id.split('@')[0];
        } catch {
            return id.split('@')[0];
    ...
    if (jid.endsWith("@g.us")) {
        ...
        if (!(v.name || v.subject)) {
            try {
                ...
                return metadata.name || metadata.subject || jid;
            } catch { return jid; }
        }
        return v.name || v.subject || jid;
    }

    if (config.areJidsSameUser(jid, sock.user.id)) {
        return sock.user.name || sock.user.verifiedName || sock.user.notify || "Bot";
    }

    v = sock.chats?.[jid] || {};
    if (withoutContact) return "";
    
    let res = v.name || v.subject || v.vname || v.notify || v.verifiedName;
    if (!res) res = getSafePhoneNumber(jid);
  ...
  sock.sendPoll = (number, property, poll) => {
    const formatPhone = (contact, full = false) => {
    ...
    const pollMessage = {
      name: property.text,
      values: poll,
      selectableCount: property.limit
    };
    return sock.sendMessage(numberClean, { poll: pollMessage, mentions: sock.parseMention(pollMessage.name) });
  };
  ...
  sock.serializeM = (m) => {
    return require("./serialize")(m, sock);
  };
  ...
  sock.sendAlbumMessage = async (jid, medias, options) => {
    ...
    for (const i in medias) {
      try {
        ...
        const img = await config.generateWAMessage(album.key.remoteJid, {
          [type]: data,
          ...(i === "0" ? {
            caption
          } : {})
