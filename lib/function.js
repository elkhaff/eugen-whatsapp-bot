...
class Function {
  saying(sock, text, m) {
    let teks = text;
    let gtts = require("./gtts")(`id`, teks);
    let ranm = "./input_voice.mp3";
    let rano = "./output_voice.ogg";
    gtts.save(ranm, teks, function () {
      exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
        fs.unlinkSync(ranm);
        let buff = fs.readFileSync(rano);
        if (err) return m.reply(`Error bang voice ai nya!`);
        sock.sendMessage(
          m.cht,
          {
            audio: buff,
  ...
  updateWinrate(db, user, game, result) {
    db.list().user[user].winrate[game] =
      db.list().user[user].winrate[game] || {};

    if (result === "win") db.list().user[user].winrate[game].win += 1;
    else if (result === "lose") db.list().user[user].winrate[game].lose += 1;
    else if (result === "tie") db.list().user[user].winrate[game].tie += 1;
  }
}
