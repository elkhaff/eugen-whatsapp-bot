...
// detail voting
const getVoteDetails = (from, data) => {
    const room = sesi(from, data);
    if (!room) return false;

    // Urutkan pemain berdasarkan jumlah suara dari terbesar ke terkecil
    const voteDetails = room.player
        .map(player => ({
            id: player.id,
            name: player.name || player.id.split("@")[0],
            votes: player.vote || 0,
...
const pagii = (data) => {
    if (data.dead.length < 1) {
        ...
        textnya = `Pagi telah tiba, warga desa menemukan ${data.dead.length > 1 ? "beberapa" : "1"} mayat di tumpukan puing dan darah berceceran. ${a ? a + " telah mati! " : ""}${d.length > 1 ? ` ${d} hampir dibunuh, namun *Guard* berhasil melindunginya.` : ""}\n\nTak terasa hari sudah siang, matahari tepat di atas kepala, terik panas matahari membuat suasana menjadi riuh\n*Hari ke ${
        data.day
    }*`;
        return textnya;
    }
...
async function voting(client, x, data) {
    ...
    textnya =
        "Senja telah tiba. Seluruh warga berkumpul di balai desa untuk memilih siapa yang akan dieksekusi. Sebagian warga terlihat sibuk menyiapkan alat penyiksaan untuk malam ini. Hati-hati, ada penghianat diantara kalian!\n\n*[ Players ]*\n";
    shortPlayer(x.room, data);
    for (let i = 0; i < x.player.length; i++) {
        if (!x.player[i].isdead) { 
            textnya += `${x.player[i].number}) @${x.player[i].id.replace("@s.whatsapp.net", "")} ${x.player[i].isdead === true ? "☠️" : ""}\n`;
            ment.push(x.player[i].id);
            rowPoll.push(`Voting player ke-${x.player[i].number}`);
        }
    }
    dayVoting(x.room, data);
    clearAll(x.room, data);
    clearAllSTATUS(x.room, data);
    await client.sendMessage(x.room, {
        text: textnya,
        mentions: ment
    });
    return await client.sendPoll(x.room, {
        text: `Silahkan vote yang kamu curigai sebagai Werewolf!`,
        limit: 1,
    },
        rowPoll
    )
...
async function malam(client, x, data) {
    ...
    let voteDetails = getVoteDetails(x.room, data); // Tambahkan untuk mendapatkan detail voting

    // Buat pesan detail vote terpisah
    let voteSummary = `*[ Detail Voting Count ]*\n`;
    let mentVote = [];
    for (let player of voteDetails) {
        if (!player.isDead) {
            if (player.votes > 0) {
                voteSummary += `\n- @${player.id.replace("@s.whatsapp.net", "")}: ${player.votes} vote`;
                mentVote.push(player.id);
            }
        }
    }

    if (hasil_vote === 0) {
        textnya = `Terlalu bimbang menentukan pilihan. Warga pun pulang ke rumah masing-masing, tidak ada yang dieksekusi hari ini. Bulan bersinar terang, malam yang mencekam telah datang. Semoga tidak ada yang mati malam ini.`;
        // await client.sendMessage(x.room, { text: voteSummary, mentions: voteDetails.map(p => p.id) }); // Kirim detail vote
        return client
            .sendMessage(x.room, {
                text: textnya,
            })
        ...
        textnya = `Warga desa telah memilih, namun hasilnya seri.\n\nBintang memancarkan cahaya indah malam ini, warga desa beristirahat di kediaman masing masing.`;
        let ment = [];
        await client.sendMessage(x.room, { text: voteSummary, mentions: mentVote }); // Kirim detail vote
        return client
            .sendMessage(x.room, {
                text: textnya,
                mentions: ment,
        ...
        if (hasil_vote.role === "werewolf") {
            ...
            textnya = `Warga desa telah memilih dan sepakat @${hasil_vote.id.replace("@s.whatsapp.net", "")} dieksekusi mati.\n\n@${hasil_vote.id.replace("@s.whatsapp.net", "")} adalah ${hasil_vote.role} ${emoji_role(hasil_vote.role)}\n\nBulan bersinar terang malam ini, warga desa beristirahat di kediaman masing masing.`;
            voteKill(x.room, data);
            let ment = [];
            ment.push(hasil_vote.id);
            await client.sendMessage(x.room, { text: voteSummary, mentions: mentVote }); // Kirim detail vote
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    let rowPoll = [];
                    for (let p = 0; p < x.player.length; p++) {
                        if (!x.player[p].isdead) {
                            rowPoll.push(`Makan player ke-${x.player[p].number}`);
                        }
                    }
                    await client.sendPoll(x.player[i].id, {
                        text: textnya,
                        limit: 1,
                    },
                        rowPoll
                ...
                if (x.player[i].isdead != true) {
                    ...
                    */
                    ...
                    for (let p = 0; p < x.player.length; p++) {
                        if (!x.player[p].isdead) {
                            rowPoll.push(`Lihat Peran player ke-${x.player[p].number}`);
                        }
                    }
                    await client.sendPoll(x.player[i].id, {
                        text: textnya,
                ...
                if (x.player[i].isdead != true) {
                    textnya = `Kamu adalah seorang *Guard*, silahkan pilih salah 1 player yang ingin kamu lindungi.`;
                    ...
                    */
                    ...
                    for (let p = 0; p < x.player.length; p++) {
                        if (!x.player[p].isdead) {
                            rowPoll.push(`Lindungi player ke-${x.player[p].number}`);
                        }
                    }
                    await client.sendPoll(x.player[i].id, {
                        text: textnya,
                ...
                if (x.player[i].isdead != true) {
                    ...
                    */
                    ...
                    for (let p = 0; p < x.player.length; p++) {
                        if (!x.player[p].isdead) {
                            rowPoll.push(`Buka Identitas player ke-${x.player[p].number}`);
                        }
                    }
                    await client.sendPoll(x.player[i].id, {
                        text: textnya,
